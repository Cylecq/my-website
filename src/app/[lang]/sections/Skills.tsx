  ...
  return (
    <section className="h-screen flex flex-col gap-2 justify-center items-center border">
      <div className="h-[25%] w-full flex items-end justify-start">
        <h2 className="text-2xl font-display px-2">Skills</h2>
      </div>

      {Object.entries(skillsData).map(([category, skills]) => (
        <div key={category} className="h-[25%] border w-11/12 bg-white border-primary shadow-inner z-10">
          <div className="h-[30%] flex justify-center items-center">
            <h3 className="text-xl">{category}</h3>
          </div>
          <div className="flex w-full h-[70%] justify-around">
            {skills.map((skill, index) => (
              <Image key={index} src={skill.src} alt={skill.alt} height={40} width={40} />
            ))}
          </div>
        </div>
      ))}
